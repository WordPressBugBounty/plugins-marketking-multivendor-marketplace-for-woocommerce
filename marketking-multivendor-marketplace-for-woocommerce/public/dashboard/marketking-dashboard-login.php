...
<html>
    <head>
        ...
        do_action('marketking_enqueue_dashboard_styles_scripts');
        ...
        /* Product Gallery Slider - Codeixer */
        if (defined('WPGS_NAME')){
            add_action('wp_print_styles', function(){
        ...
        // Flexible Shipping, not yet working, cannot display rules
        /*
        if (defined('FLEXIBLE_SHIPPING_VERSION')){

            add_action('wp_print_styles', function(){

                $assets_url = WP_CONTENT_URL . '/plugins/flexible-shipping/assets';
                $suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

                wp_enqueue_style(
                    'fs_admin',
                    trailingslashit( $assets_url ) . 'css/admin.css',
                    [],
                    2
                );
                wp_enqueue_style(
                    'fs_font',
                    trailingslashit( $assets_url ) . 'css/font' . $suffix . '.css',
                    [],
                    2
                );
                wp_enqueue_style(
                    'fs_rules_settings',
                    trailingslashit( $assets_url ) . 'css/rules-settings.css',
                    [],
                    2
                );
            ...
            add_action('wp_print_scripts', function(){
                ...
                wp_register_script(
                    'fs_admin',
                    trailingslashit( $assets_url ) . 'js/admin' . $suffix . '.js',
                    [ 'jquery' ],
                    2
                );
                ...
                wp_register_script(
                    ...
                    trailingslashit( $assets_url ) . 'js/contextual-info' . $suffix . '.js',
                    [ 'jquery' ],
                    2
                );
                ...
                wp_register_script(
                    'fs_rules_settings',
                    trailingslashit( $assets_url ) . 'js/rules-settings.js',
                    [ 'wp-i18n' ],
                    2
                );
            ...
            if (class_exists('WC_Subscriptions')){
                ...
                add_action('wp_print_scripts', function(){
                    ...
                    if ($order){
                        ...
                        $subscr = wcs_get_subscription( $order_id );
        ...
        if (apply_filters('marketking_enable_pricebasedcountry_integration', true)){
            ...
            if ( defined('WCPBC_PLUGIN_FILE') ) {
                ...
                add_action('wp_print_styles', function(){
                    ...
                    wp_enqueue_style( 'wcpbc_admin_styles', WCPBC()->plugin_url() . 'assets/css/admin/admin' . $suffix . '.css', array(), WCPBC()->version );
                ...
                add_action('wp_print_scripts', function(){
                   ...
                   // new
                   // JS.
                   wp_register_script( 'wcpbc_admin', WCPBC()->plugin_url() . 'assets/js/admin/wcpbc_admin' . $suffix . '.js', array( 'jquery', 'woocommerce_admin', 'accounting' ), WCPBC()->version, true );
                   wp_register_script( 'wcpbc_notices', WCPBC()->plugin_url() . 'assets/js/admin/notices' . $suffix . '.js', array( 'jquery' ), WCPBC()->version, true );
                   wp_register_script( 'wcpbc_system_report', WCPBC()->plugin_url() . 'assets/js/admin/system-report' . $suffix . '.js', array( 'jquery' ), WCPBC()->version, false );
                   ...
                   wp_enqueue_script( 'wcpbc_notices' );
                   wp_enqueue_script( 'wcpbc_admin' );
