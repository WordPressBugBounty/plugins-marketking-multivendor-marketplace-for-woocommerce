...
(function($){
	...
	$( document ).ready(function() {
		...
		var buttonclass = 'marketking-btn-gray';
	            ...
	            columnDefs: [
	                {
	                  ...
	                  render: function(data, type, row, meta) {
                          if (type === 'display') {
                            return data; // Keep HTML for display in the table
                          } else {
                            // For exporting, replace <br> with \n and strip other HTML tags
	                  ...
	                  exportOptions: {
	                    ...
	                    format: {
	                      body: function (data, row, column, node) {
	                        // Apply custom formatting only to the 6th column (index 5)
	                        if (column === 6) {
	                          return data.replace(/<br\s*\/?>/gi, '\n')
	                                     .replace(/<\/?[^>]+(>|$)/g, "");
	                        }
	                        // For other columns, use the default formatting
	                        return $.fn.dataTable.Buttons.stripData(data);
	                      }
	                    }
	                  }
	                },
	                ...
	                },
	                {
	                  ...
	                  customize: function(win) {
	                    $(win.document.body).find('table')
	                      .addClass('print-friendly')
	                      .css('font-size', '12px');
	                    
	                    $(win.document.body).append('<style>' +
	                      '.print-friendly td:nth-child(6) { white-space: pre-wrap; }' +
	                      '</style>'
	                    );
	                  }
	                },
